...
def accuracy(typed, source):
    ...
    if total_words == 0:
        ...
        correct_words = sum(w1 == w2 for w1, w2 in zip(typed_words, source_words))
...
def wpm(typed, elapsed):
    ...
    "*** YOUR CODE HERE ***"
    character = len(typed)
    typing_words = 12.0 * character / elapsed
    return typing_words
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        array += [diff_function(typed_word, word, limit)]
        if word == typed_word:
            return typed_word

        mini = min(array)
        find = array.index(mini)

    if mini > limit:
        return typed_word
    else:
        return word_list[find]
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    """measure = len(source) - len(typed)
    if typed == source:
        return 0
    elif not typed:  # typed为空，直接返回measure
        return measure
    elif not source:  # source为空，说明需要将typed中的所有字符都替换掉
        return len(typed)
    elif typed[0] == source[0]:
        return feline_fixes(typed[1:], source[1:], limit)
    else:
        substitute = 1 + feline_fixes(typed[1:], source[1:], limit - 1)
        if substitute >= limit:
            substitute = limit
        return substitute"""
...
def time_per_word(words, times_per_player):
    ...
    >>> get_all_words(match)
    ['collar', 'plush', 'blush', 'repute'][[3, 2, 1], [4, 2, 3]]
    >>> get_all_times(match)
    [[6, 3, 6, 2], [10, 6, 1, 2]][[6, 3, 6, 2], [10, 6, 1, 2]]
    """
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
