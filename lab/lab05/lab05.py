...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    front = max(get_lat(city_a), get_lat(city_b)) - min(get_lat(city_a), get_lat(city_b))
    after = max(get_lon(city_a), get_lon(city_b)) - min(get_lon(city_a), get_lon(city_b))
    return sqrt(front**2 + after**2)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    city_c = make_city('location', lat, lon)
    a = distance(city_a, city_c)
    b = distance(city_b, city_c)
    if a < b:
